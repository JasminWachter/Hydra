...
This document describes how the full MITRE-aligned rule set is generated.
...
The generated full rule set (`full_interaction_rules.P`) remains available for
explicit override scenarios that require broader MITRE technique coverage.

The provenance rule sets are intentionally retained unchanged. Corresponding files ending in
`_positive.P` are conservative monotonic positive-body variants generated by
`MulVAL/kb/tools/generate_positive_variants.py`: positive disjunctions are expanded, while
alternatives requiring negation-as-failure, Prolog cut, or unsafe head variables are omitted.
These variants retain MulVAL/XSB wrappers and supported built-ins and are therefore not claimed to
be pure standalone Datalog or semantically equivalent to the evaluated originals.
